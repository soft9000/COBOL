...
start-changeformat.
    ...
    move spaces to record-type dont-convert
    move 'FIXED' to format-type
    if 'FREE' = upper-case(change)
...
*>      remove the comment
        ...
        subtract 1 from wordx-max end-subtract
    end-if
...
*>      check to see if we need to break the line
        ...
        evaluate true
        when wordx < 1
*>          there's no break point
            move word-values(1) to scan-values
            move 66 to break-point
...
*>              double-quoted literal
            ...
            when other
*>              probably a comment which we could truncate
*>              but we won't
                display 'unable to break' end-display
                display line-number space input-record end-display
                perform abort-run
            end-evaluate
            move work-record to input-record
